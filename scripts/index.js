...
let brainrots = [
  ...
  { id: 15, nome: "LA GRANDE COMBINASION", custo: 4500, power: 50, img: "./imgs/LA-GRANDE.gif"}
];

let seuBrainrot = brainrots[0];

let pets = [
  { nome: "Coelho", preco: 100, bonus: 1 },
  { nome: "Gato", preco: 150, bonus: 2 },
  { nome: "Cachorro", preco: 500, bonus: 3 }
];

function aplicarPets() {
  // soma todos os bônus dos pets e aplica aos brainrots
  let totalBonus = seusPets.reduce((soma, p) => soma + p.bonus, 0);
  brainrots.forEach(b => {
    b.power = b.powerBase + totalBonus; // usa o valor base original
  });
}

// Salvar o valor base original dos brainrots (para evitar stack infinito de bônus)
brainrots.forEach(b => b.powerBase = b.power);
...
if (localStorage.getItem("clickerSave")) {
  ...
  aplicarPets(); // aplica o bônus dos pets ao carregar
}
...
peteggbtn.addEventListener("click", () => {
  ...
  if (clicks >= pet.preco) {
    clicks -= pet.preco;
    seusPets.push({ nome: pet.nome, bonus: pet.bonus });
    aplicarPets();
...
function ResetGame() {
  ...
  seusPets = [];
  brainrots.forEach(b => b.power = b.powerBase);
  clickmsg.textContent = `Clicks: ${clicks}`;
  multimsg.textContent = `Multiplicador: ${seuBrainrot.power}X`;
  brainrotIMG.src = seuBrainrot.img;
  brainrotmsg.textContent = `Brainrots: ${seuBrainrot.nome}`;
